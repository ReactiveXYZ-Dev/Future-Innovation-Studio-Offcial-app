...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView == __confirmAlert) {
        ...
        UIAlertView* migrationAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Testing Network", nil) message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles:nil];
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
            ...
            __completeAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Testing Complete", nil) message:NSLocalizedString(@"Testing Complete Message", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
...
+ (void)showAndWaitForMigration:(void (^)(void))migration completion:(void (^)(void))completion
{
    if (migration) {
        ...
        void (^startMigrationBlock)(void) = ^{
            ...
            __confirmAlert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"testing Start", nil) message:NSLocalizedString(@"Testing Start Message", nil) delegate:__migrationDelegate cancelButtonTitle:NSLocalizedString(@"OK", nil) otherButtonTitles:nil];
            [__confirmAlert show];
