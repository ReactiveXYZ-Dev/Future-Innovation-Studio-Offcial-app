...
- (CGSize)titleLabelSizeForTitleText:(NSString *)titleText {
    ...
    CGFloat lineHeight = [titleText sizeWithAttributes:@{NSFontAttributeName: self.titleLabel.font}].height;
    ...
    for (CGFloat targetWidth = MMProgressHUDMinimumWidth; numberOfLines > 2; targetWidth += targetWidthIncrementor) {
        ...
        titleSize = [titleText boundingRectWithSize:CGSizeMake(targetWidth, 500.f) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.titleLabel.font} context:nil].size;
...
- (CGSize)statusSizeForMessageText {
    ...
    for (CGFloat targetWidth = MMProgressHUDMinimumWidth; statusSize.width < statusSize.height + additiveHeightConstant; targetWidth += targetWidthIncrementor) {
        if(targetWidth >= MMProgressHUDMaximumWidth)
            break;
        statusSize = [self.messageText boundingRectWithSize:CGSizeMake(targetWidth, 500.f) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.statusLabel.font} context:nil].size;
    }
    return statusSize;
}
